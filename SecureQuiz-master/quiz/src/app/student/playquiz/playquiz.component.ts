...
export class PlayquizComponent implements OnInit {
  ...
  getScore() {
    console.log(this.ansOfOneQuestion);
    console.log(this.ansKey);
    const totalQuestions = this.ansKey.length;
  
    for (let i = 0; i < totalQuestions; i++) {
      const item1 = this.ansOfOneQuestion[i];
      const item2 = this.ansKey[i];
  
      if (item1.qid === item2.qid && item1.ans === item2.ans) {
        this.score++;
      }
    }
  
    this.percentagescore = (this.score / totalQuestions) * 100;
    this.finalscore =  `Score: ${this.score}/${totalQuestions}, Percentage: ${this.percentagescore}%`;
  }
